//This project was written by Daniel Cardenas, Haemin Lee, Stephanie Contreras and Imon Daneshmand.
...
void help(){
	printf("\nserver command:\n 'server -t <file name> -i <routing-update-interval>'\n"); 
	printf("update command\n 'update <server-id> <server-id> <link cost>'\n");
	printf("step command\n 'step'\n");
	printf("packets command\n 'packets'\n");
...
//TESTING TODO
void notifyNeighbors(int crashedServerID)
{
	//Send distance vector to all neighbors
	int k;
	for (k = 0; k < 4; k++)
	{
		if (neighbor[k] > 0)
		{
			...
			char temp_a[256];
			char messageArray[50];
			sprintf(messageArray, "%d %d %d %d %d %s", array[myServID - 1][0], array[myServID - 1][1], array[myServID - 1][2], array[myServID - 1][3], crashedServerID, "e");
			//send message
			send(socket_test, messageArray, 50, 0);
			//clearing messageArray
			memset(messageArray, 0, sizeof(messageArray));
			...
			if (FD_ISSET(sd, &readfds) && (sd > 0))
			{
				...
				if (rval > 0) //a message is being sent
				{
					...
					{ //neighbors link has been disabled
						printf("\nThe link has been DISABLED from Server %d, updating my vector\n", fromServ);
					...
					{ //neighbors link crashed
						...
						printf("\nThey have Crashed at Server %d, updating my vector\n", fromServ);
						//update disabled neighbor link cost AND update array at my server's row
						neighbor[fromServ - 1] = -1;
						array[myServID - 1][0] = neighbor[0];
						array[myServID - 1][1] = neighbor[1];
						...
						//update array row for that server (fromServ)  TODO WORKING
						array[fromServ - 1][0] = -1;
						array[fromServ - 1][1] = -1;
						array[fromServ - 1][2] = -1;
						array[fromServ - 1][3] = -1;
						//step();  //TODO
						notifyNeighbors(fromServ);  //TODO
						step();  //TODO
					}
					else if ((strcmp(check_if_disabled, "e")) == 0)
					{
						// "fromServ" is actually the crashed server
						array[0][fromServ - 1] = -1;
						array[1][fromServ - 1] = -1;
						array[2][fromServ - 1] = -1;
						array[3][fromServ - 1] = -1;
